...
The Harvester integration lets MXCuBE treat a **CrystalDirect Harvester 3** (CDH3)
machine as the source of samples instead of a conventional cryogenic dewar.
The CDH3 cuts crystals directly from a crystallisation plate, mounts them on a
...
| **Room temperature**      | Crystal mounted without cryo-cooling; sample changer collects the pin directly without waiting for the transfer state  |

The active mode is controlled by `get_room_temperature_mode()` /
`set_room_temperature_mode()` on the `Harvester` object. The web route for
`set_room_temperature_mode` additionally synchronises the same flag on the
sample changer and the diffractometer in a single call.

## Full data pipeline

The CDH3 sits in a broader software pipeline:
...
- **ISPyB / DRAC** stores the diffraction data and results downstream of
  MXCuBE.
...
| Class                  | Module                                    | Role                                                         |
| ---------------------- | ----------------------------------------- | ------------------------------------------------------------ |
| `Harvester`            | `HardwareObjects/Harvester.py`            | Low-level exporter interface to the CDH3 machine             |
...
`Harvester` owns the direct calls to the CDH3; `HarvesterMaintenance` and
`EMBLFlexHarvester` use it through the `harvester` beamline role.

The common beamline roles are:

- `harvester` — the hardware object that talks to the CDH3 device.
...
classDiagram
    ...
    class Harvester {
        ...
        +get_offsets_for_sample_centring() tuple
...
All CDH3 communication in `Harvester` goes through `_execute_cmd_exporter()`,
which wraps both exporter **commands** (sent via `add_command`) and exporter
**attributes** (read/written via `add_channel`).

## Harvester States

`HarvesterState` defines the possible states of the CDH3 machine.
...
exporter-based CDH3 hardware object. During `init()` it reads the
...
- **Calibration and centring** — `load_calibrated_pin()`,
  ...
  `get_offsets_for_sample_centring()`, and drift/cut-shape offset accessors.

Two CDH3 status strings act as important state boundaries:

- `"Ready"` — the CDH3 can accept the next command.
- `"Waiting Sample Transfer"` — a harvested crystal is ready for the sample
  changer.

`_wait_sample_transfer_ready()` handles a timeout by aborting the current CDH3
...
configuration:
  exporter_address: wid30harvest:9001
...
sequenceDiagram
    ...
    participant CDH3 as CDH3 exporter
    ...
    Flex->>HA: get_offsets_for_sample_centring()
    Flex->>MD: start_harvester_centring(offsets)
```
...
flowchart TD
    ...
    OK([Return True]) --> SC([SC loads Sample X])
```
...
sequenceDiagram
    ...
    HA->>CDH3: harvestCrystal(next_uuid)
```
...
sequenceDiagram
    ...
    HA->>CDH3: loadCalibratedPin [exporter cmd]
    HA->>HA: _wait_sample_transfer_ready(None)
    Note over HM: Wait 40 s (CDH3 reports ready ~40 s early)
    HM->>SC: load_a_pin_for_calibration()
    SC->>CDH3: loadSampleFromHarvester(pin_cleaning) [exporter cmd]
    CDH3-->>SC: Pin mounted
    HM->>MD: move_motors(kappa, kappa_phi, phi, phiy + drift_x)
    HM->>MD: centringFocus.set_value_relative(drift_z)
    HM->>MD: centringVertical.set_value_relative(drift_y)
    HM->>MD: save_current_motor_position()
    ...
    HM->>MD: Read current motor positions
    ...
    HA->>CDH3: storePinToBeamOffset(x, y, z) [exporter cmd]
...
`Harvester.get_offsets_for_sample_centring()` computes the motor offsets needed
...
centringFocus         = drift_z                + pin_to_beam[0]
centringTableVertical = drift_y  - cut_shape_y + pin_to_beam[2]
```
...
| -------------------------------------- | --------------- | ----------------------------------------------------------- |
| `get_state()`                          | `str`           | Raw state string from CDH3 (`"Ready"`, `"Running"`, …)      |
| `get_status()`                         | `str`           | Human-readable status string                                |
| `_ready()`                             | `bool`          | `True` when state is `"Ready"`                              |
| `_busy()`                              | `bool`          | `True` when state is not `"Ready"`                          |
| `_ready_to_transfer()`                 | `bool`          | `True` when status is `"Waiting Sample Transfer"`           |
...
| Method                              | Returns        | Description                                                 |
| ----------------------------------- | -------------- | ----------------------------------------------------------- |
| `load_calibrated_pin()`             | `None`         | Ask CDH3 to prepare a calibration pin                       |
| `store_calibrated_pin(x, y, z)`     | `None`         | Write focus/phiy/phiz offsets (mm) to CDH3                  |
...
| `get_offsets_for_sample_centring()` | `tuple[float]` | `(phiy_offset, centringFocus, centringTableVertical)` in mm |
...
| `start_harvester_centring()`             | `None`        | Fetch offsets and trigger automatic centring on the diffractometer                                                                           |
| `harvest_and_mount_sample(uuid, sample)` | `bool`        | One-shot interactive harvest and mount                                                                                                       |
| `get_room_temperature_mode()`            | `bool`        | Reads room-temperature mode from the **sample changer's own exporter** (independent of the CDH3 exporter)                                    |
...
- **CRIMS reporting** — `send_data_collection_info_to_crims()`.
...
| `GET`  | `/send_data_collection_info_to_crims` | Upload data-collection info to CRIMS    |
...
- **Procedure** section:
  ...
  - **Send Data to Crims** button → `sendDataCollectionToCrims()`.
